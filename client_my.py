...
class Client:
    def __init__(self, host, port, timeout=None):
        ...
        # созда. клиентский сокет, запоминаем объект socke.socket в self
        self.host = host
        self.port = port
        try:
    ...
    def _read(self):
        ...
        data = b""
        # накапливаю буфер, пока не встретим "\n\n" в конце команды
    ...
    def get(self, key):
        # формирую и отправляю запрос команды get
        try:
            self.connection.sendall(
                f"get {key}\n".encode()
            )
        ...
        # разбираю ответ для команды get
